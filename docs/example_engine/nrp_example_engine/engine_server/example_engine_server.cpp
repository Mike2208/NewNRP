...
#include "nrp_example_engine/python/create_device_class.h"
...
nlohmann::json ExampleEngineServer::initialize(const nlohmann::json &data, EngineJSONServer::lock_t&)
{
	...
	const ExampleConfig config(data.at(ExampleConfig::ConfigType.m_data));
	...
	try
	{
		...
		for(long i=0; i < numDevices; ++i)
		{
			...
			auto devController = std::shared_ptr<ExampleEngineDeviceController<ExampleDevice> >(new
			            ExampleEngineDeviceController<ExampleDevice>(DeviceIdentifier(devName, config.engineName(), ExampleDevice::TypeName.data()),
	...
	return nlohmann::json({{ExampleConfig::InitFileExecStatus, true}, {ExampleConfig::InitFileParseDevMap, jsonDevMap}});
}
...
{
	return nlohmann::json({{ExampleConfig::InitFileExecStatus, 0}, {ExampleConfig::InitFileErrorMsg, errMsg}});
}
